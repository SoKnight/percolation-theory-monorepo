...
class Bonds(val size: Int) {
    ...
    /** Число рёбер, у которых оба конца заняты: только на них разыгрываются связи. Записывает генератор. */
    ...
    /** Доля занятых связей от рёбер с обоими занятыми концами, `0`, если таких рёбер нет. */
    val availableConcentration: Double
        get() = if (available == 0) 0.0 else occupied.toDouble() / available
