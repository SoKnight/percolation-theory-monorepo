 ...
 */
fun Bonds.writeAsPNG(sites: Lattice, pSite: Double, pBond: Double, trial: Int, path: Path = defaultPathPNG(pSite, pBond, trial)): Path {
...
/** Путь по умолчанию для картинки: `out/site_bond_L{L}_s{p_site}_b{p_bond}_t{испытание}.png`. */
fun Bonds.defaultPathPNG(pSite: Double, pBond: Double, trial: Int): Path =
    Path("out", "site_bond_L${size}_s${pSite.format()}_b${pBond.format()}_t$trial.png")
